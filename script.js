...
// ==========================================
// 5. CHAT SYSTEM (IMPROVED UI & VIP SYSTEM)
// ==========================================
function initCustomChatList() {
    ...
    chatContainer.innerHTML = `
        <div id="chatWrapper" style="background:#0a0a0a; color:#fff; height:100dvh; min-height:100vh; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width:480px; margin:0 auto; display:flex; flex-direction:column; position:relative; overflow:hidden;">
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; padding:16px 20px; border-bottom:1px solid #1f1f1f; background:#121212; flex-shrink:0;">
                <span style="font-size:1.3rem; font-weight:800; color:#fff; letter-spacing:-0.5px;">Direct Messages</span>
                <i class="fa-regular fa-pen-to-square" style="font-size:1.2rem; color:#e50914; cursor:pointer;"></i>
            </div>
            ...
            <div id="singleRoomView" style="display:none; flex-direction:column; height:100dvh; background:#0a0a0a; position:fixed; top:0; left:0; right:0; bottom:0; z-index:9999; max-width:480px; margin:0 auto; overflow:hidden;">
                <div id="singleRoomHeader" style="display:flex; align-items:center; gap:12px; padding:12px 16px; border-bottom:1px solid #1f1f1f; background:#121212; flex-shrink:0;"></div>
                ...
                <form id="chatForm" onsubmit="sendCustomMessage(event)" style="padding:12px 16px; display:flex; align-items:center; gap:10px; border-top:1px solid #1f1f1f; flex-shrink:0; background:#121212; margin:0;">
                    <input type="text" id="customChatInput" placeholder="Write a message..." autocomplete="off" style="flex:1; background:#1e1e1e; border:1px solid #2d2d2d; color:#fff; padding:12px 18px; border-radius:24px; font-size:0.9rem; outline:none;">
                    <button type="submit" style="background:#e50914; border:none; color:#fff; font-weight:bold; font-size:0.9rem; cursor:pointer; padding:10px 18px; border-radius:24px; transition:opacity 0.2s;">Send</button>
                </form>
            </div>
        </div>
    `;
...
function renderChatRows() {
    ...
    mockChats.forEach(chat => {
        ...
        const fontStyle = chat.hasUnread ? "font-weight:700; color:#e50914;" : "font-weight:normal; color:#8e8e93;";
        ...
        row.innerHTML = `
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-left:12px;">
                ${chat.hasUnread ? `<span style="width:10px; height:10px; background:#e50914; border-radius:50%; display:inline-block;"></span>` : `<span style="font-size:0.75rem; color:#666;">${chat.time}</span>`}
            </div>
        `;
...
function renderRoomMessages(chat) {
    ...
    chat.messages.forEach(m => {
        ...
        bubble.style.cssText = isSent ?
            "background:#e50914; color:#fff; padding:12px 16px; border-radius:18px 18px 2px 18px; max-width:75%; font-size:0.9rem; word-break:break-word; box-shadow: 0 2px 5px rgba(0,0,0,0.2);" :
            "background:#1f1f1f; color:#fff; padding:12px 16px; border-radius:18px 18px 18px 2px; max-width:75%; font-size:0.9rem; word-break:break-word;";
        ...
        if (m.vipWarning) {
            ...
            warning.style.cssText = "margin-top:8px; background:rgba(229, 9, 20, 0.12); border:1px solid #e50914; color:#ff6b6b; padding:10px 14px; border-radius:12px; font-size:0.8rem; display:flex; align-items:center; justify-content:space-between; gap:10px; max-width:85%; width:100%; box-sizing:border-box;";
            warning.innerHTML = `
                <div style="display:flex; align-items:center; gap:8px;">
                    <i class="fa-solid fa-lock" style="color:#e50914;"></i>
                    <span>VIP membership required to chat.</span>
                </div>
                <button onclick="openPaymentModal()" style="background:#e50914; color:#fff; border:none; padding:6px 12px; border-radius:8px; font-size:0.75rem; font-weight:bold; cursor:pointer; flex-shrink:0;">Upgrade</button>
            `;
...
// ==========================================
// 6. BOTTOM SHEET / MODALS FOR MODELS & GROUPS
// ==========================================
function openModelCard(model) {
    ...
    if (!sheet) {
        ...
        sheet.style.cssText = "position:fixed; bottom:0; left:0; right:0; height:50vh; background:#141414; border-top-left-radius:20px; border-top-right-radius:20px; border-top:1px solid #282828; z-index:10000; display:flex; flex-direction:column; align-items:center; padding:20px; box-shadow:0 -10px 30px rgba(0,0,0,0.8); transition:transform 0.3s ease-in-out; transform:translateY(100%); max-width:480px; margin:0 auto;";
    ...
    sheet.innerHTML = `
        ...
        <div style="position:relative; margin-bottom:12px;">
            <img src="${model.img}" style="width:90px; height:90px; border-radius:50%; object-fit:cover; border:2px solid #e50914;">
            <span style="position:absolute; bottom:4px; right:4px; width:14px; height:14px; background:${model.isOnline ? '#34c759' : '#8e8e93'}; border:2px solid #141414; border-radius:50%;"></span>
        </div>
        <h3 style="margin:0; font-size:1.3rem; color:#fff;">${model.title}</h3>
        <p style="margin:6px 0; color:#a0a0a0; font-size:0.9rem;">
            <i class="fa-solid fa-location-dot" style="color:#e50914;"></i> ${model.city || "Addis Ababa"} • ${model.age || 22} Y/O
        </p>
        <span style="background:#222; color:#ccc; font-size:0.8rem; padding:4px 12px; border-radius:12px; margin-bottom:20px;">${model.sub}</span>
        
        <button onclick="handleVIPAction('You must be a VIP member to start a chat with this model!')" style="width:100%; background:#e50914; color:#fff; border:none; padding:12px; border-radius:25px; font-weight:bold; font-size:1rem; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px;">
...
function openGroupCard(group) {
    ...
    if (!sheet) {
        ...
        sheet.style.cssText = "position:fixed; bottom:0; left:0; right:0; height:45vh; background:#141414; border-top-left-radius:20px; border-top-right-radius:20px; border-top:1px solid #282828; z-index:10000; display:flex; flex-direction:column; align-items:center; padding:20px; box-shadow:0 -10px 30px rgba(0,0,0,0.8); transition:transform 0.3s ease-in-out; transform:translateY(100%); max-width:480px; margin:0 auto;";
        document.body.appendChild(sheet);
    }
    ...
    sheet.innerHTML = `
        ...
        <img src="${group.img}" style="width:80px; height:80px; border-radius:16px; object-fit:cover; margin-bottom:12px; border:1px solid #333;">
        ...
        <button onclick="handleVIPAction('You must be a VIP member to join this private group!')" style="width:100%; background:#e50914; color:#fff; border:none; padding:12px; border-radius:25px; font-weight:bold; font-size:1rem; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px;">
...
function handleVIPAction(message) {
    alert(message);
    closeModelCard();
    closeGroupCard();
    if (!currentUser) {
